...
def hashdir(fileDir):
    ...
    for filename in os.listdir(fileDir):
        ...
        if file_extension(filename) == '.dds':
            oldfilename1 = GetFileNameAndExt(filename)+'.dds';
            oldfilename2 = GetFileNameAndExt(filename)+'.tga';
            newfilename = md5.hexdigest()+'.tga';
            print(oldfilename1)
            pass
        else:
            oldfilename1 = filename;
            oldfilename2 = filename;
            newfilename = md5.hexdigest()+file_extension(filename);
        ...
        for fileObj in fileList:
            ...
            for line in all_the_lines:
                f.write(line.replace(oldfilename1,newfilename).replace(oldfilename2,newfilename)) 
            f.close()
